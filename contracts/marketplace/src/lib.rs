...
impl MarketplaceContract {
    ...
    fn require_compliance_attested(env: &Env, seller: &Address) -> Result<(), KoraError> {
        let config = Self::load_config(env)?;
        let registry_client =
            kora_risk_registry::RiskRegistryContractClient::new(env, &config.risk_registry);
        if !registry_client.is_compliance_attested(seller) {
